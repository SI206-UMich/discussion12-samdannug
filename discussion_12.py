...
def add_employee(filename, cur, conn):
    ...
    for d in data:
        ...
        job_id = d["job_id"]
        salary = d["salary"]
        cur.execute('insert or ignore into employees (employee_id, first_name, last_name, job_id, hire_date, salary) values (?, ?, ?, ?, ?, ?)', (emp_id, f_name, l_name, job_id, hire_d, salary))
    conn.commit()
...
def problematic_salary(cur, conn):
    cur.execute('select employees.first_name, employees.last_name from jobs join employees on jobs.job_id = employees.job_id where employees.salary < jobs.min_salary or employees.salary > jobs.max_salary')
    x = cur.fetchall()
    return x
